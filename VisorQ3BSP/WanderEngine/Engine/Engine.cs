...
namespace WanderEngine
{
    ...
    public class Engine
    {
        ...
        private ForceCollection Forces = new ForceCollection();
        ...
        private Texture KnifeTexture = null;
        private ArrayList LaserBeams = new ArrayList();
        private float LaserTime = 0f;
        private NetworkPlayerState LastState = new NetworkPlayerState(float.NaN, float.NaN, float.NaN, float.NaN);
        ...
        private double fieldOfView = 45.0f;
        private double nearClipDistance = 1.0f;
        private double farClipDistance = 5000.0f;
        ...
        static Vector3f side = new Vector3f();
        static Vector3f up = new Vector3f();
        ...
        public Engine(GameWindow gameWin)
        {
            ...
            gameWin.Mouse.ButtonDown += new EventHandler<MouseButtonEventArgs>(gameWin_Mouse_ButtonDown);

            this.InitializeAll();
        }
		
        bool acquireMouse = true;
        
        void gameWin_Mouse_ButtonDown(object sender, MouseButtonEventArgs e)
        {
        ...
        public static String Quake3FilesPath
        {
            get;
            set;
        }
        ...
        {
            int num = this.Generator.Next(this.SpawnPoints.Count);
            Vector3f newPos = (Vector3f) this.SpawnPoints[num];
            this.MapCamera.MoveCameraTo(newPos);
            this.MapCamera.MoveCameraUpDown(12f);
        }
        ...
        */
        ...
        {
            lock (this.IncomingPlayers)
            {
                this.IncomingPlayers.Add(new NetworkPlayer(e.ID, e.Name, e.ModelName, e.InitialState, e.IsMarked));
            }
        }
        ...
        {
            lock (this.OutgoingPlayerIDs)
            {
                this.OutgoingPlayerIDs.Add(e.ID);
            }
        }
        ...
        public void Fire()
        {
            if (!this.IsDead && (this.WeaponTime <= 0f))
            {
                Hashtable hashtable;
                if (this.Weapon == WeaponType.Knife)
                {
                    ...
                    this.StabTime = 0.3f;
                    Vector3f a = this.MapCamera.ProjectPosition(5f, 0f);
                    lock ((hashtable = this.NetworkPlayers))
                    {
                        foreach (NetworkPlayer player in this.NetworkPlayers.Values)
                        {
                            if ((Vector3f.Distance(a, player.PlayerModel.Position) < 45.0) && (player.PlayerModel.ModelState != AnimationState.DeathFallFoward))
                            {
                                // miki-sound-off this.EngineSoundManager.PlaySound("pain");
                                if (this.m_Connected)
                                {
                                    this.SendHurt(player.ID);
                                }
                ...
                if (this.Weapon == WeaponType.Laser)
                {
                    this.LaserTime = 0.9f;
                    if (this.Ammo <= 0)
                    {
                    ...
                    else
                    {
                        ...
                        this.Ammo--;
                        this.AmmoChanged(this, null);
                        this.CurrentMap.DetectCollisionRay(this.MapCamera.Position, this.MapCamera.ProjectPositionWithY(10000f, 0f));
                        Vector3f start = new Vector3f(this.MapCamera.Position.X, this.MapCamera.Position.Y, this.MapCamera.Position.Z);
                        LaserBeamInfo info = new LaserBeamInfo();
                        info.Start = this.MapCamera.ProjectPositionWithY(0f, 3f);
                        info.Start2 = this.MapCamera.ProjectPositionWithY(0f, 5f);
                        info.End = new Vector3f(this.CurrentMap.CollisionInfo.EndPoint.X, this.CurrentMap.CollisionInfo.EndPoint.Y, this.CurrentMap.CollisionInfo.EndPoint.Z);
                        info.TimeRemaining = 0.3f;
                        lock (this.LaserBeams)
                        {
                            this.LaserBeams.Add(info);
                        ...
                        lock ((hashtable = this.NetworkPlayers))
                        {
                            foreach (NetworkPlayer player2 in this.NetworkPlayers.Values)
                            {
                                if (player2.PlayerModel.RayCollides(start, info.End))
                                {
                                    ...
                                    if (this.m_Connected)
                                    {
                                        this.SendHurt(player2.ID);
                                    }
                                    player2.AddColorMask(1f, 0f, 0f, 0.3f);
                                }
                            }
                        }
                        float angle = Utility.CapAngle(90f - this.MapCamera.Yaw);
                        this.Forces[this.BlastForce].Direction.X = -Utility.CosDeg(angle);
                        this.Forces[this.BlastForce].Direction.Y = Utility.SinDeg(this.MapCamera.Pitch);
                        this.Forces[this.BlastForce].Direction.Z = Utility.SinDeg(angle);
                        this.Forces[this.BlastForce].SetVelocity(this.Forces[this.BlastForce].MaxVelocity.X, 0f, this.Forces[this.BlastForce].MaxVelocity.Z);
                    }
                }
            }
        }
        ...
        {
            foreach (Item item2 in this.Items.Values)
            {
                if (item2.Active && (Vector3f.Distance(this.MapCamera.Position, item2.Position) <= 25.0))
                {
                    return item2;
                }
            }
        ...
        private void InitializeAll()
        {
            ...
            this.KnifeTexture = new Texture(Quake3FilesPath + 
                                            Utility.AdaptRelativePathToPlatform("textures/bookstore/knife.jpg"));
            this.textFont = new Text2D(Quake3FilesPath + Utility.AdaptRelativePathToPlatform("fonts/NeHe.Lesson17.Font.bmp"));
        }

        private void InitializeForces()
        {
            this.RunForce = this.Forces.Add(new Force());
            this.Forces[this.RunForce].MinVelocity = new Vector3f(0f, 0f, 0f);
            this.Forces[this.RunForce].MaxVelocity = new Vector3f(300f, 300f, 300f);
            this.Forces[this.RunForce].Acceleration = new Vector3f(-80000f, -80000f, -80000f);
            this.StrafeForce = this.Forces.Add(new Force());
            this.Forces[this.StrafeForce].MinVelocity = new Vector3f(0f, 0f, 0f);
            this.Forces[this.StrafeForce].MaxVelocity = new Vector3f(300f, 300f, 300f);
            this.Forces[this.StrafeForce].Acceleration = new Vector3f(-150000f, -150000f, -150000f);
            this.GravityForce = this.Forces.Add(new Force());
            this.Forces[this.GravityForce].MaxVelocity = new Vector3f(0f, 1500f, 0f);
            this.Forces[this.GravityForce].Direction.Y = -4.5f;
            this.JumpForce = this.Forces.Add(new Force());
            this.Forces[this.JumpForce].MaxVelocity = new Vector3f(0f, 500f, 0f);
            this.Forces[this.JumpForce].Acceleration.Y = -40000f;
            this.Forces[this.JumpForce].Direction.Y = 1f;
            this.BlastForce = this.Forces.Add(new Force());
            this.Forces[this.BlastForce].MaxVelocity = new Vector3f(200f, 0f, 200f);
            this.Forces[this.BlastForce].Acceleration = new Vector3f(-22000f, 0f, -22000f);
        }
        ...
        {   /* miki-play-off 
            ...
            */
            ...
            this.HealthChanged = (EventHandler)Delegate.Combine(this.HealthChanged, new EventHandler(this.Engine_HealthChanged));
            this.ServerTimeChanged = (EventHandler) Delegate.Combine(this.ServerTimeChanged, new EventHandler(this.Engine_ServerTimeChanged));
            this.ScoreChanged = (EventHandler) Delegate.Combine(this.ScoreChanged, new EventHandler(this.Engine_ScoreChanged));
            this.GameOver = (GameOverHandler) Delegate.Combine(this.GameOver, new GameOverHandler(this.Engine_GameOver));
            this.RoundInfoChanged = (EventHandler) Delegate.Combine(this.RoundInfoChanged, new EventHandler(this.Engine_RoundInfoChanged));
            this.AmmoChanged = (EventHandler) Delegate.Combine(this.AmmoChanged, new EventHandler(this.Engine_AmmoChanged));
            this.WeaponChanged = (EventHandler) Delegate.Combine(this.WeaponChanged, new EventHandler(this.Engine_WeaponChanged));
            this.QuitGame = (EventHandler) Delegate.Combine(this.QuitGame, new EventHandler(this.Engine_QuitGame));
        }
...
/*
        ...
        public void LoadMap(string DirectoryPath, string MapName)
        {
            ...
            this.CurrentMap = new BSPFile(MapName);
            GL.MatrixMode(MatrixMode.Modelview);
            GL.LoadIdentity();
            GL.MatrixMode(MatrixMode.Projection);
            GL.LoadIdentity();
            BSPEntity[] entityArray = this.CurrentMap.Entities.SeekEntitiesByClassname("info_player_deathmatch");
            foreach (BSPEntity entity in entityArray)
            {
                string[] strArray = entity.SeekValuesByArgument("origin");
                if (strArray.Length > 0)
                {
                    string[] strArray2 = Regex.Split(strArray[0], " ");
                ...
                string[] strArray3 = entityArray[0].SeekValuesByArgument("angle");
                if (strArray3.Length > 0)
                {
                    try
                    {
                        float num = float.Parse(strArray3[0]);
                        this.MapCamera.Yaw = 90f - num;
                    }
                    catch
                    {
                    }
        ...
        private void OnResizeControl()
        {
            ...
            GL.MatrixMode(MatrixMode.Projection);
            GL.LoadIdentity();
            float num = (winWidth + 0.1f) / ((float) winHeight);
            gluPerspective(70.0, (double) num, 1.0, 5000.0);
            GL.MatrixMode(MatrixMode.Modelview);
            GL.LoadIdentity();
        }
        ...
        {            
            ...
            {
                Hashtable hashtable;
                this.PrepareRenderMap();
                this.MapCamera.UpdateView();
                this.MapFrustrum.UpdateFrustrum();
                this.CurrentMap.RenderLevel(this.MapCamera.Position, this.MapFrustrum);
                lock ((hashtable = this.Items))
                {
                    ...
                    foreach (Item item in this.Items.Values)
                    {
                        ...
                        if (item.AlphaDir)
                        {
                            item.Alpha += 0.5f * this.TimeElapsed;
                            if (item.Alpha > 1f)
                            {
                                item.Alpha = 1f;
                                item.AlphaDir = false;
                ...
                lock ((hashtable = this.NetworkPlayers))
                {
                    foreach (NetworkPlayer player in this.NetworkPlayers.Values)
                    {
                        player.Update(this.TimeElapsed);
                ...
                if ((this.HurtTime > 0f) || (this.DeathTime > 0f))
                {
                    this.HurtTime -= this.TimeElapsed;
                    GL.PushMatrix();
                    GL.LoadIdentity();
                    GL.Enable(EnableCap.Blend);
  ...
  //GL.BlendFunc(BlendingFactorSrc.?, BlendingFactorDest.DstColor);
                ...
                lock (this.LaserBeams)
                {
                    for (int i = 0; i < this.LaserBeams.Count; i++)
                    {
                        LaserBeamInfo info = (LaserBeamInfo) this.LaserBeams[i];
                        if (info.TimeRemaining > 0f)
                        {
                            info.TimeRemaining -= this.TimeElapsed;
                ...
                if (this.Weapon == WeaponType.Knife)
                {
                    ...
                    GL.Disable(EnableCap.CullFace);
                    GL.DepthMask(false);
                    GL.BindTexture(TextureTarget.Texture2D, this.KnifeTexture.TextureID);
                    if (this.StabTime > 0f)
                    {
                        this.StabTime -= this.TimeElapsed;
                        GL.Begin(BeginMode.Triangles);
                        GL.TexCoord2(0f, 0f);
                ...
                {
                    if (this.LaserTime > 0f)
                    {
                        this.LaserTime -= this.TimeElapsed;
                    }
                    GL.PushMatrix();
                    GL.Disable(EnableCap.DepthTest);
                    GL.Disable(EnableCap.CullFace);
        ...
        public void Respawn()
        {
            ...
            this.Ammo = 10;
            this.AmmoChanged(this, null);
            this.Forces.ClearDisposableForces();
            this.ChooseSpawnPoint();
        }
        ...
        {
            float num = 90f - this.MapCamera.Yaw;
            this.LastState.X = this.MapCamera.Position.X;
            this.LastState.Y = this.MapCamera.Position.Y;
            this.LastState.Z = this.MapCamera.Position.Z;
            this.LastState.Yaw = this.MapCamera.Yaw;
            /* miki-play-off 
            NetworkPacket sendData = new NetworkPacket();
            sendData.Write(3);
            sendData.Write(this.m_NetworkID);
        ...
        {
            Vector3f vector = new Vector3f();
            for (Vector3f vector2 = new Vector3f(ProjectedPos.X, ProjectedPos.Y, ProjectedPos.Z); ((Math.Abs(vector2.X) > 0.0001f) || (Math.Abs(vector2.Y) > 0.0001f)) || (Math.Abs(vector2.Z) > 0.0001f); vector2 = new Vector3f(vector.X, vector.Y, vector.Z))
            {
                this.CurrentMap.DetectCollisionBox(SourceVector, SourceVector + vector2, this.PlayerState.BoundMin, this.PlayerState.BoundMax);
                if (this.CurrentMap.CollisionInfo.Fraction > 0f)
                {
                    this.MapCamera.MoveCameraTo(this.CurrentMap.CollisionInfo.EndPoint);
                    return true;
                }
                Vector3f vector3 = new Vector3f();
                vector3.CopyFrom(this.CurrentMap.CollisionInfo.Normal);
                vector3.Normalize();
                vector = vector2 - ((Vector3f) (vector3 * vector3.Dot(vector2)));
            }
            return false;
        }
        ...
        {
            this.CurrentMap.DetectCollisionBox(this.MapCamera.Position, this.MapCamera.Position + this.PlayerState.Step, this.PlayerState.BoundMin, this.PlayerState.BoundMax);
            if (this.CurrentMap.CollisionInfo.Fraction == 0f)
            {
                return false;
            }
            if (!this.TrySlide(this.CurrentMap.CollisionInfo.EndPoint, ProjectedPos))
            {
                return false;
            }
            this.CurrentMap.DetectCollisionBox(this.MapCamera.Position, this.MapCamera.Position - this.PlayerState.Step, this.PlayerState.BoundMin, this.PlayerState.BoundMax);
        ...
        public void UpdateFrame(float TimeElapsed)
        {
            ...
            if (!this.m_IsPaused)
            {
                if (this.CurrentMap != null)
                {
                    ArrayList list;
                    Hashtable hashtable;
                    lock ((list = this.IncomingPlayers))
                    {
                        foreach (NetworkPlayer player in this.IncomingPlayers)
                        {
                            ...
                            {
                                player.PlayerModel.Position = new Vector3f(player.InitialState.X, 0f, player.InitialState.Z);
                            ...
                            lock ((hashtable = this.NetworkPlayers))
                            {
                                this.NetworkPlayers[player.ID] = player;
                                continue;
                            }
                        }
                        this.IncomingPlayers.Clear();
                    }
                    lock ((list = this.OutgoingPlayerIDs))
                    {
                        foreach (int num in this.OutgoingPlayerIDs)
                        {
                            lock ((hashtable = this.NetworkPlayers))
                            {
                                if (this.NetworkPlayers.ContainsKey(num))
                                {
                                    this.NetworkPlayers.Remove(num);
                                }
                                continue;
                            }
                        }
                        this.OutgoingPlayerIDs.Clear();
                    }
                    this.UpdateInput(ref this.PlayerState);
                    if (this.IsDead)
                    {
                        this.DeathTime -= TimeElapsed;
                    ...
                    {
                        this.UpdatePlayerState(this.PlayerState);
                        Item itemCollision = this.GetItemCollision();
                    ...
                    if (this.m_Connected && this.NextStateReady)
                    {
                        this.SendMovementState();
                    }
                }
                ...
                GL.Enable(EnableCap.Blend);
                textFont.glPrint(10, 20, "Right mouse button enable-disable cursor capture.", 0);
            }
        }
        ...
        {/*
            ...
            if (this.m_ProcessInput)
            {
                this.ProcessKeyboardEvents();
                this.ProcessMouseEvents(ref State);
            }
        }
        ...
        private void ProcessKeyboardEvents()
        {
            try
            {
                ...
                {
                    float angle = Utility.CapAngle(90f - this.MapCamera.Yaw);
                    this.Forces[this.RunForce].Direction.X = Utility.CosDeg(angle);
                    this.Forces[this.RunForce].Direction.Z = -Utility.SinDeg(angle);
                    this.Forces[this.RunForce].AddVelocityX(this.Forces[this.RunForce].MaxVelocity.X);
                    this.Forces[this.RunForce].AddVelocityZ(this.Forces[this.RunForce].MaxVelocity.Z);
                }
                else if (gameWin.Keyboard[ControlCfg.MoveBack])
                {
                    float num2 = Utility.CapAngle(90f - this.MapCamera.Yaw);
                    this.Forces[this.RunForce].Direction.X = -Utility.CosDeg(num2);
                    this.Forces[this.RunForce].Direction.Z = Utility.SinDeg(num2);
                    this.Forces[this.RunForce].AddVelocityX(this.Forces[this.RunForce].MaxVelocity.X);
                    this.Forces[this.RunForce].AddVelocityZ(this.Forces[this.RunForce].MaxVelocity.Z);
                }
                if (gameWin.Keyboard[ControlCfg.LeftStrafe])
                {
                    float num3 = Utility.CapAngle(-this.MapCamera.Yaw);
                    this.Forces[this.StrafeForce].Direction.X = -Utility.CosDeg(num3);
                    this.Forces[this.StrafeForce].Direction.Z = Utility.SinDeg(num3);
                    this.Forces[this.StrafeForce].AddVelocityX(this.Forces[this.StrafeForce].MaxVelocity.X);
                    this.Forces[this.StrafeForce].AddVelocityZ(this.Forces[this.StrafeForce].MaxVelocity.Z);
                }
                else if (gameWin.Keyboard[ControlCfg.RightStrafe])
                {
                    float num4 = Utility.CapAngle(-this.MapCamera.Yaw);
                    this.Forces[this.StrafeForce].Direction.X = Utility.CosDeg(num4);
                    this.Forces[this.StrafeForce].Direction.Z = -Utility.SinDeg(num4);
                    this.Forces[this.StrafeForce].AddVelocityX(this.Forces[this.StrafeForce].MaxVelocity.X);
                    this.Forces[this.StrafeForce].AddVelocityZ(this.Forces[this.StrafeForce].MaxVelocity.Z);
                }
                if (this.EnableJump)
                {
                    if (gameWin.Keyboard[ControlCfg.Jump] && (this.Forces[this.GravityForce].GetVelocityY() <= 0f))
                    {
                        this.EnableJump = false;
                        // miki-sound-off this.EngineSoundManager.PlaySound("jump");
                        this.Forces[this.JumpForce].AddVelocityY(this.Forces[this.JumpForce].MaxVelocity.Y);
                    }
                }
        ...
        private void ProcessMouseEvents(ref CommandSet State)
        {
            try
            {
                ...
                if(acquireMouse)
                {
                	...
                	this.PlayerState.PlayerLook.X = -pointer_delta.Width;

                	if (this.ControlCfg.InvertMouse)
                	{
                    	this.PlayerState.PlayerLook.Y = pointer_delta.Height;
                	}
                	else
                	{
                    	this.PlayerState.PlayerLook.Y = -pointer_delta.Height;
        ...
        {
            float num = this.TimeElapsed * this.ControlCfg.MouseSpeedHorz;
            float num2 = this.TimeElapsed * this.ControlCfg.MouseSpeedVert;
            this.MapCamera.SetMouseView(PlayerState.PlayerLook.X * num, PlayerState.PlayerLook.Y * num2);
            this.CurrentMap.DetectCollisionBox(this.MapCamera.Position, this.MapCamera.Position - PlayerState.Step, PlayerState.BoundMin, PlayerState.BoundMax);
            if (this.CurrentMap.CollisionInfo.Fraction > 0f)
            {
                if (this.Forces[this.GravityForce].GetVelocityY() <= 0f)
                {
                    this.Forces[this.GravityForce].Acceleration.Y = 20000f;
                }
            }
            else
            {
                this.Forces[this.GravityForce].Acceleration.Y = 0f;
                this.Forces[this.GravityForce].SetVelocity(0f, 0f, 0f);
            }
            Vector3f sourcePoint = new Vector3f(this.MapCamera.Position.X, this.MapCamera.Position.Y, this.MapCamera.Position.Z);
            lock (this.Forces)
            {
                this.Forces.ApplyAllForces(sourcePoint, this.TimeElapsed);
            }
            Vector3f projectedPos = sourcePoint - this.MapCamera.Position;
            if (!this.TryStepUpAndSlide(projectedPos))
            {
                this.TrySlide(this.MapCamera.Position, projectedPos);
            }
        }
